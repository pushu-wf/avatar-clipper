export class EventBus<EventMap> {
	...
	public on<K extends string & keyof EventMap>(eventName: K, callback: EventMap[K]) {
	...
	public emit<K extends string & keyof EventMap>(eventName: K, payload?: EventMap[K] extends (payload: infer P) => void ? P : never) {
		if (!eventName) return;
	...
	public off<K extends string & keyof EventMap>(eventName: K, callback: EventMap[K]) {
		if (!eventName || typeof callback !== "function") return;
		const callBackSet = this.eventHub.get(eventName);
		if (!callBackSet) return;
